...
def navigate_to_target(client_pc, target_point, tolerance=100):
    is_robot_moving = False

    while True:
        # Simulated robot position and direction
        robot_pos, robot_direction = (900, 600), (1, 0)  # Adjust as needed to a unit vector

        print(f"robot_pos: {robot_pos}, type: {type(robot_pos)}")
        print(f"robot_direction: {robot_direction}, type: {type(robot_direction)}")
        print(f"target_point: {target_point}, type: {type(target_point)}")

        if not isinstance(robot_pos, tuple):
            raise ValueError(f"robot_pos should be a tuple but got {type(robot_pos)}")
        if not isinstance(target_point, tuple):
            raise ValueError(f"target_point should be a tuple but got {type(target_point)}")
        if not isinstance(robot_direction, tuple):
            raise ValueError(f"robot_direction should be a tuple but got {type(robot_direction)}")

        if are_points_close(robot_pos, target_point, tolerance=tolerance):
            client_pc.send_command("stop")
            is_robot_moving = False
            break

        angle = rotate_vector_to_point(robot_pos, robot_direction, target_point)
        print(f"Calculated angle: {angle}, type: {type(angle)}")


        angle_tolerance = 10
        if angle < -angle_tolerance or angle > angle_tolerance:
            if is_robot_moving:
                client_pc.send_command("stop")
                is_robot_moving = False
